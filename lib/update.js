...
// 执行更新
async function execUpdate(hasPerm, domains) {
  ...
  if (hasPerm) {
    ...
    logger.success('The hosts file updated successfully.\n')
    await flushDNSService().catch(() => {
      logger.warn(
        'Maybe you need to restart your computer for these changes to take effect.\n'
      )
    })
    ...
    try {
      ...
      return chalk.cyan(
        `The host entry has been copied to the clipboard.\n  You could paste it to the hosts file (${chalk.yellow(
          DEFAULT_HOSTS
        )}).`
      )
    } catch (e) {}
